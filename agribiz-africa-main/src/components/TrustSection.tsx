...
const TrustSection = () => {
  ...
  const getColorClasses = (color: string) => {
    const colors: Record<string, { bg: string; text: string; border: string }> = {
      ...
      lime: { bg: 'bg-lime-100 dark:bg-lime-900/30', text: 'text-lime-600 dark:text-lime-400', border: 'border-lime-200 dark:border-lime-800' },
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {trustMetrics.map((metric, index) => {
            ...
            return (
              <div
                key={index}
                className={`bg-white dark:bg-slate-700 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-300 border ${colors.border} animate-fade-in-up`}
              ...
              >
                ...
                <div className="text-gray-900 dark:text-gray-100 font-medium text-sm mb-1">
                  {metric.label}
                </div>
